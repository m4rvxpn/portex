...
// NULLScanner performs NULL scans (-sN): no TCP flags set.
...
// NewNULLScanner creates a new NULLScanner.
func NewNULLScanner(builder *PacketBuilder, rawSock *RawSocket, capture *Capturer, srcIP net.IP) *NULLScanner {
	return &NULLScanner{
		builder: builder,
		rawSock: rawSock,
		capture: capture,
...
// Scan performs a NULL scan against dst:dstPort.
func (s *NULLScanner) Scan(ctx context.Context, dst net.IP, dstPort, srcPort int, ttl uint8) (scanner.PortState, string, time.Duration, error) {
	if srcPort == 0 {
	...
	// No flags set
	frame, err := s.builder.BuildTCP(dst, dstPort, srcPort, TCPFlags{}, ttl, nil)
	if err != nil {
